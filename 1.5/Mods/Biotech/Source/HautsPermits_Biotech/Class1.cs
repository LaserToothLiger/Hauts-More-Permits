...
namespace HautsPermits_Biotech
{
    ...
    public class QuestNode_EcosphereIntermediary : QuestNode_Sequence
    {
        ...
        {
            return HVMP_Utility.TryFindEcosphereFaction(out Faction ecosphereFaction);
        }
    }
    //case study
    ...
    public class Hediff_MiasmaticRot : Hediff
    {
        ...
        {
            base.Tick();
            if (this.pawn.IsHashIntervalTick(2000) && this.pawn.SpawnedOrAnyParentSpawned)
            {
    ...
    public class NaturePreserve : Thing
    {
        public override void Tick()
        {
            if (this.Spawned && this.IsHashIntervalTick(177))
            {
                if (this.AnyBadJuju)
                {
    ...
    public class HediffComp_Brightdeath : HediffComp
    {
        ...
        public override void Notify_PawnDied(DamageInfo? dinfo, Hediff culprit = null)
        {
            ...
            {
                this.Props.sound.PlayOneShot(new TargetInfo(this.Pawn.PositionHeld, this.Pawn.MapHeld, false));
    ...
    public class HediffComp_FrenzyTimer : HediffComp
    {
        ...
        {
            base.CompPostTick(ref severityAdjustment);
            this.curPhaseDuration--;
            if (this.curPhaseDuration <= 0)
            {
                if (this.parent.CurStageIndex == 0)
                {
                    if (this.Pawn.IsHashIntervalTick(500) && Rand.Chance(this.Props.frenzyChancePerHourFifth))
                    {
